...
async def get_simulation_preset(well_id: str, db: AsyncSession = Depends(get_db)):
    ...
    current_sor = prod.sor if prod and prod.sor else 0.22
    ...
    rod_load = srp.polished_rod_load_kn if (srp and srp.polished_rod_load_kn) else (srp.rod_load_max_kn if (srp and srp.rod_load_max_kn) else 16.9)
    current_cycle_num = css.cycle_number if (css and css.cycle_number) else 4
    ...
    return {
        ...
        "data": {
            ...
            "current": {
                "cycleNumber": current_cycle_num,
                "nextCycleNumber": next_cycle_num,
                "cssStatus": f"Cycle {current_cycle_num} (Late Decline Phase)",
                "steamVolumeTon": css.steam_volume_ton if css else 1169.3,
                "injectionPressureBar": css.steam_injection_pressure_bar if css else 80.0,
                "soakTimeHr": css.soak_time_hr if css else 144.0,
                "spm": srp.spm if srp else 5.1,
                "strokeLengthIn": srp.stroke_length_in if srp else 66.0,
                "vfdFrequencyHz": srp.vfd_frequency_hz if srp else 36.0,
                "rodLoad": round(rod_load, 1),
            ...
            "optimized": {
                "cycleNumber": next_cycle_num,
                "steamVolumeTon": round((css.steam_volume_ton if css else 1169.3) * 1.15, 1),
                "injectionPressureBar": round((css.steam_injection_pressure_bar if css else 80.0) * 0.95, 1),
                "soakTimeHr": round((css.soak_time_hr if css else 144.0) * 0.88, 1),
                "spm": 5.1,
                "strokeLengthIn": 66.0,
                "vfdFrequencyHz": 36.0,
                "rodLoad": round(rod_load * 0.88, 1),
                "oilProduction": round(current_oil * 1.28, 1),
                "sor": round(current_sor * 0.835, 2),
            }
        }
    }
