...
import { 
  ...
  Sliders, TrendingUp, Users, Zap, Check, X, RotateCcw
} from 'lucide-react';
...
export const OptimizationPage: React.FC<OptimizationPageProps> = ({ tab = 'joint' }) => {
  ...
  // Approval Subtabs & State (Screenshot 2)
  const [approvalSubTab, setApprovalSubTab] = useState<'pending' | 'approved' | 'rejected' | 'audit'>('pending');
  const [approvalList, setApprovalList] = useState([
    { id: '1', date: '04 Nov 14:20', wellId: 'BGW-014', rec: 'Reduce SPM to 5.1', impact: '+14% Production', status: 'Pending', comment: 'Looks good. Within operating limits. -- Field Engineer' },
    { id: '2', date: '04 Nov 12:15', wellId: 'BGW-021', rec: 'Adjust steam volume', impact: '-8% SOR', status: 'Approved', comment: 'Approved by Field Manager' },
    { id: '3', date: '03 Nov 16:40', wellId: 'BGW-007', rec: 'Increase soak time', impact: '+11% Production', status: 'Pending', comment: 'Thermal simulation verification requested' },
    { id: '4', date: '03 Nov 11:30', wellId: 'BGW-003', rec: 'Optimize VFD', impact: '-7% Energy', status: 'Rejected', comment: 'Motor drive current fluctuation exceeds tolerance' },
    { id: '5', date: '02 Nov 14:22', wellId: 'BGW-005', rec: 'Adjust injection pressure', impact: '+9% Production', status: 'Approved', comment: 'Approved by Lead Engineer' },
  ]);
  const [activeReviewWell, setActiveReviewWell] = useState('BGW-014');
  const [engineerComment, setEngineerComment] = useState('Looks good. Within operating limits. -- Field Engineer');

  // SRP Optimization Parameters State (Screenshot 3)
  ...
  // CSS Optimization Parameters State (Screenshot 5)
  ...
  const handleApproveWell = (wellId: string) => {
    setApprovalList(prev => prev.map(item => item.wellId === wellId ? { ...item, status: 'Approved' } : item));
  };

  const handleRejectWell = (wellId: string) => {
    setApprovalList(prev => prev.map(item => item.wellId === wellId ? { ...item, status: 'Rejected' } : item));
  };
                  ...
                  {approvalList
                    ...
                    .map(item => (
                      <tr key={item.id} className="hover:bg-[#F8FAFC] transition-colors">
                        ...
                        <td className="py-3.5 px-4 text-[#334155]">{item.rec}</td>
                        <td className="py-3.5 px-4 font-bold text-[#15803D]">{item.impact}</td>
                        ...
                        <td className="py-3.5 px-4 text-center">
                          {item.status === 'Pending' ? (
                            <button
                              onClick={() => {
                                setActiveReviewWell(item.wellId);
                                setEngineerComment(item.comment);
                              }}
          ...
          <div className="bg-white rounded border border-[#E2E8F0] shadow-sm p-6 space-y-4">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3 w-full md:w-auto justify-end">
                <button
                  onClick={() => handleApproveWell(activeReviewWell)}
                  className="flex items-center gap-1.5 px-6 py-2.5 bg-[#065F46] hover:bg-[#047857] text-white font-bold text-[13px] rounded transition-colors shadow-xs cursor-pointer"
                >
                  <Check className="w-4 h-4" />
                  <span>Approve</span>
                </button>
                <button
                  onClick={() => handleRejectWell(activeReviewWell)}
                  className="flex items-center gap-1.5 px-6 py-2.5 bg-[#DC2626] hover:bg-[#B91C1C] text-white font-bold text-[13px] rounded transition-colors shadow-xs cursor-pointer"
                >
                  <X className="w-4 h-4" />
                  <span>Reject</span>
                </button>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[12px] text-[#64748B]">Recommendation ID: <strong className="text-[#0F172A]">REC-2026-0814</strong></span>
                </div>
                <h3 className="text-xl font-black text-[#0F172A] mt-1">Well BGW-014 (Baghewala Sand Member A)</h3>
                <p className="text-[13px] text-[#64748B]">
                  Simultaneously optimize Cycle 15 steam injection with increased volume and dampen SRP pumping speed.
                </p>
              </div>
            </div>
            ...
            <div className="bg-white rounded border border-[#E2E8F0] shadow-sm p-6 space-y-4">
              ...
              <div className="space-y-3 text-[13px]">
                <div className="flex justify-between py-1.5 border-b border-[#F8FAFC]">
                  ...
                  <span className="font-bold text-[#0F172A]">Cycle 14 (Late Decline Phase)</span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-[#F8FAFC]">
                  <span className="text-[#64748B]">Steam Volume:</span>
                  <span className="font-bold text-[#0F172A]">800 metric tons</span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-[#F8FAFC]">
                  <span className="text-[#64748B]">Pumping Speed (SPM):</span>
                  <span className="font-bold text-[#DC2626]">10.0 SPM (Fluid Pound)</span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-[#F8FAFC]">
                  <span className="text-[#64748B]">Polished Rod Load:</span>
                  <span className="font-bold text-[#DC2626]">6.3 kN (High Stress)</span>
                </div>
                <div className="flex justify-between py-1.5">
                  <span className="text-[#64748B]">Oil Production:</span>
                  <span className="font-bold text-[#0F172A]">24.8 BOPD</span>
                </div>
              </div>
            </div>

            <div className="bg-white rounded border-2 border-[#D32F2F] shadow-sm p-6 space-y-4">
              <h4 className="text-[16px] font-bold text-[#D32F2F] pb-3 border-b border-[#FFEBEE]">AI Recommended Coupled Setpoints</h4>
              <div className="space-y-3 text-[13px]">
                <div className="flex justify-between py-1.5 border-b border-[#F8FAFC]">
                  <span className="text-[#64748B]">Recommended CSS:</span>
                  <span className="font-bold text-[#15803D]">Cycle 15 Injection (1,100 tons)</span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-[#F8FAFC]">
                  <span className="text-[#64748B]">Optimized SPM:</span>
                  <span className="font-bold text-[#15803D]">7.5 SPM (Eliminates Fluid Pound)</span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-[#F8FAFC]">
                  <span className="text-[#64748B]">Expected Rod Load:</span>
                  <span className="font-bold text-[#15803D]">5.1 kN (Safe Fatigue Envelope)</span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-[#F8FAFC]">
                  <span className="text-[#64748B]">Projected Oil Rate:</span>
                  <span className="font-black text-[#15803D] text-[15px]">34.4 BOPD (+9.6 BOPD)</span>
                </div>
                <div className="flex justify-between py-1.5">
                  <span className="text-[#64748B]">Projected SOR:</span>
                  <span className="font-black text-[#15803D]">3.1 bbl/bbl (-18.4%)</span>
                </div>
              </div>
            </div>
          </div>
