...
export const FieldStatusPage: React.FC = () => {
  ...
  const totalWellsCount = displayWells.length || BAGHEWALA_MAP_WELLS.length;
  const criticalCount = displayWells.filter(w => w.status.toUpperCase() === 'CRITICAL').length;
  const attentionCount = displayWells.filter(w => w.status.toUpperCase() === 'ATTENTION').length;
  const normalCount = totalWellsCount - criticalCount - attentionCount;

  const criticalWellsList = displayWells.filter(w => w.status.toUpperCase() === 'CRITICAL').map(w => w.id);
